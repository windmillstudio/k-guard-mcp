...
def test_identical_data07_materializations_are_narrow_fix(tmp_path) -> None:
...
def test_data07_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    (first[3] / "slots" / SLOT_ID / "candidate-0" / "fixed" / "app" / "api" / "checkout" / "orders.py").write_text(
        "tampered\n", encoding="ascii"
    )
