...
from scripts.materialize_l3_auth_rls_db_api07 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api07_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-07"
    ...
    vulnerable = materialization_root / "slots" / "api-07" / "candidate-0" / "vulnerable" / "src" / "client" / "supabase.ts"
    fixed = materialization_root / "slots" / "api-07" / "candidate-0" / "fixed" / "src" / "server" / "supabase-admin.ts"
    negative = materialization_root / "slots" / "api-07" / "candidate-1" / "negative" / "src" / "status.ts"
    reserve_vulnerable = materialization_root / "slots" / "api-07" / "candidate-1" / "vulnerable" / "src" / "browser" / "supabase.ts"
    reserve_fixed = materialization_root / "slots" / "api-07" / "candidate-1" / "fixed" / "src" / "server" / "supabase-admin.ts"
    assert b"NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY" in vulnerable.read_bytes()
    assert b"createClient(" in vulnerable.read_bytes()
    assert b"server-only" not in vulnerable.read_bytes()
    assert b"NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY" not in fixed.read_bytes()
    assert b'import "server-only";' in fixed.read_bytes()
    assert b"SUPABASE_SERVICE_ROLE_KEY" in fixed.read_bytes()
    assert b"VITE_SUPABASE_SERVICE_ROLE_KEY" in reserve_vulnerable.read_bytes()
    assert b"createClient(" in reserve_vulnerable.read_bytes()
    assert b"VITE_SUPABASE_SERVICE_ROLE_KEY" not in reserve_fixed.read_bytes()
    assert b'import "server-only";' in reserve_fixed.read_bytes()
    assert b"SUPABASE_SERVICE_ROLE_KEY" in reserve_fixed.read_bytes()
    assert b'export const status = "ok";' in negative.read_bytes()
    assert b"supabase" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY" not in receipt_raw
    assert b"src/client/supabase.ts" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api07_materialization_candidates_invalid"):
    ...
    with pytest.raises(ValueError, match="api07_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    fixed.write_bytes(original_fixed)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api07_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api07_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api07_materialization_root_must_be_external"):
        materialize_api07(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api07_materialization_receipt_output_must_be_external"):
        materialize_api07(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api07_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api07(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api07_materialization_root"):
        materialize_api07(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api07_materialization_receipt"):
        materialize_api07(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
