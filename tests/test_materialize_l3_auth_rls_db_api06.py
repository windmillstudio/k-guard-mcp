...
from scripts.materialize_l3_auth_rls_db_api06 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api06_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-06"
    ...
    vulnerable = materialization_root / "slots" / "api-06" / "candidate-0" / "vulnerable" / "src" / "routes" / "profiles.js"
    fixed = materialization_root / "slots" / "api-06" / "candidate-0" / "fixed" / "src" / "routes" / "profiles.js"
    negative = materialization_root / "slots" / "api-06" / "candidate-1" / "negative" / "src" / "status.ts"
    reserve_vulnerable = materialization_root / "slots" / "api-06" / "candidate-1" / "vulnerable" / "src" / "routes" / "team-members.js"
    reserve_fixed = materialization_root / "slots" / "api-06" / "candidate-1" / "fixed" / "src" / "routes" / "team-members.js"
    assert b'router.patch("/profiles/:profileId"' in vulnerable.read_bytes()
    assert b"Object.assign(profile, req.body);" in vulnerable.read_bytes()
    assert b"const update = {" not in vulnerable.read_bytes()
    assert b"Object.assign(profile, req.body);" not in fixed.read_bytes()
    assert b"Object.assign(profile, update);" in fixed.read_bytes()
    assert b"displayName: req.body.displayName," in fixed.read_bytes()
    assert b"locale: req.body.locale," in fixed.read_bytes()
    assert b'router.patch("/team-members/:memberId"' in reserve_vulnerable.read_bytes()
    assert b"Object.assign(member, req.body);" in reserve_vulnerable.read_bytes()
    assert b"Object.assign(member, req.body);" not in reserve_fixed.read_bytes()
    assert b"Object.assign(member, update);" in reserve_fixed.read_bytes()
    assert b"notificationOptIn: req.body.notificationOptIn," in reserve_fixed.read_bytes()
    assert b'export const status = "ok";' in negative.read_bytes()
    assert b"express" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"profiles" not in receipt_raw
    assert b"src/routes/profiles.js" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api06_materialization_candidates_invalid"):
    ...
    with pytest.raises(ValueError, match="api06_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    fixed.write_bytes(original_fixed)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api06_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api06_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api06_materialization_root_must_be_external"):
        materialize_api06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api06_materialization_receipt_output_must_be_external"):
        materialize_api06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api06_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api06_materialization_root"):
        materialize_api06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api06_materialization_receipt"):
        materialize_api06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
