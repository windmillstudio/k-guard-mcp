...
from scripts.compare_l3_auth_rls_db_api12_repeats import compare_api12_repeats, write_comparison
from scripts.materialize_l3_auth_rls_db_api12 import materialize_api12
...
def test_identical_api12_materializations_are_narrow_fix(tmp_path) -> None:
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    fixed = first[3] / "slots" / "api-12" / "candidate-0" / "fixed" / "src" / "main" / "java" / "com" / "kguard" / "generated" / "AdminController.java"
    fixed.write_text("tampered\n", encoding="ascii")
    with pytest.raises(ValueError, match="api12_materialization_source_content_invalid"):
        _compare(first, second)

    with pytest.raises(ValueError, match="api12_repeat_inputs_must_be_external"):
        compare_api12_repeats(
    ...
    with pytest.raises(ValueError, match="api12_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_api12_repeat_comparison"):
