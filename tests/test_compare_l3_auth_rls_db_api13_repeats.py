...
from scripts.compare_l3_auth_rls_db_api13_repeats import compare_api13_repeats, write_comparison
from scripts.materialize_l3_auth_rls_db_api13 import materialize_api13
...
def test_identical_api13_materializations_are_narrow_fix(tmp_path) -> None:
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api13_repeat_inputs_must_be_external"):
        compare_api13_repeats(
    ...
    with pytest.raises(ValueError, match="api13_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_api13_repeat_comparison"):
