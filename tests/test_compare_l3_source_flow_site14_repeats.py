...
from scripts.compare_l3_source_flow_site14_repeats import compare_site14_repeats, write_comparison
from scripts.materialize_l3_source_flow_site14 import materialize_site14
...
def test_identical_site14_materializations_are_narrow_fix(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    result = compare_site14_repeats(
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site14_repeat_inputs_must_be_external"):
        compare_site14_repeats(
    ...
    with pytest.raises(ValueError, match="site14_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_site14_repeat_comparison"):
