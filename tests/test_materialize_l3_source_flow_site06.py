...
from scripts.materialize_l3_source_flow_site06 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site06_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-06"
    ...
    vulnerable = materialization_root / "slots" / "site-06" / "candidate-0" / "vulnerable" / "routes" / "preview.js"
    fixed = materialization_root / "slots" / "site-06" / "candidate-0" / "fixed" / "routes" / "preview.js"
    negative = materialization_root / "slots" / "site-06" / "candidate-1" / "negative" / "routes" / "avatar.js"
    assert b"const rawTarget = req.query.url;" in vulnerable.read_bytes()
    assert b"await httpClient.get(rawTarget)" in vulnerable.read_bytes()
    assert b'target.protocol !== "https:"' in fixed.read_bytes()
    assert b'target.hostname !== "preview.internal.test"' in fixed.read_bytes()
    assert b"target.username" in fixed.read_bytes()
    assert b"target.password" in fixed.read_bytes()
    assert b'target.port !== "443"' in fixed.read_bytes()
    assert b'httpClient.get("https://cdn.internal.test/avatar.png")' in negative.read_bytes()
    assert b"req.query" not in negative.read_bytes()
    assert b"imageUrl" not in negative.read_bytes()
    assert b"httpClient.get" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site06_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site06_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site06_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site06_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site06_materialization_root_must_be_external"):
        materialize_site06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site06_materialization_receipt_output_must_be_external"):
        materialize_site06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site06_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site06_materialization_root"):
        materialize_site06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site06_materialization_receipt"):
        materialize_site06(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
