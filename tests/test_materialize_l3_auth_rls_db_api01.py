...
from scripts.materialize_l3_auth_rls_db_api01 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api01_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-01"
    ...
    vulnerable = materialization_root / "slots" / "api-01" / "candidate-0" / "vulnerable" / "routes" / "invoices.js"
    fixed = materialization_root / "slots" / "api-01" / "candidate-0" / "fixed" / "routes" / "invoices.js"
    negative = materialization_root / "slots" / "api-01" / "candidate-1" / "negative" / "routes" / "healthz.js"
    assert b'router.get("/api/invoices/:invoiceId"' in vulnerable.read_bytes()
    assert b"const requesterId = req.user.id;" in vulnerable.read_bytes()
    assert b"invoiceRepository.findById(invoiceId);" in vulnerable.read_bytes()
    assert b"invoiceRepository.findByIdForOwner(invoiceId, requesterId);" in fixed.read_bytes()
    assert b"findById(invoiceId);" not in fixed.read_bytes()
    assert b'router.get("/healthz"' in negative.read_bytes()
    assert b"req.user" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"invoiceRepository" not in receipt_raw
    assert b"/api/invoices" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api01_materialization_candidates_invalid"):
    ...
    with pytest.raises(ValueError, match="api01_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    fixed.write_bytes(original_fixed)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api01_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api01_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api01_materialization_root_must_be_external"):
        materialize_api01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api01_materialization_receipt_output_must_be_external"):
        materialize_api01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api01_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api01_materialization_root"):
        materialize_api01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api01_materialization_receipt"):
        materialize_api01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
