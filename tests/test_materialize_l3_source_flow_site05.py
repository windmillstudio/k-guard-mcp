...
from scripts.materialize_l3_source_flow_site05 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site05_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-05"
    ...
    vulnerable = materialization_root / "slots" / "site-05" / "candidate-0" / "vulnerable" / "app" / "api" / "continue" / "route.ts"
    fixed = materialization_root / "slots" / "site-05" / "candidate-0" / "fixed" / "app" / "api" / "continue" / "route.ts"
    negative = materialization_root / "slots" / "site-05" / "candidate-1" / "negative" / "app" / "api" / "signin" / "route.ts"
    assert b'searchParams.get("next")' in vulnerable.read_bytes()
    assert b"NextResponse.redirect(new URL(rawTarget, request.url))" in vulnerable.read_bytes()
    assert b"candidate.origin !== requestOrigin" in fixed.read_bytes()
    assert b'new URL("/welcome", request.url)' in fixed.read_bytes()
    assert b'new URL("/dashboard", request.url)' in negative.read_bytes()
    assert b"searchParams" not in negative.read_bytes()
    assert b"returnTo" not in negative.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site05_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site05_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site05_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site05_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site05_materialization_root_must_be_external"):
        materialize_site05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site05_materialization_receipt_output_must_be_external"):
        materialize_site05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site05_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site05_materialization_root"):
        materialize_site05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site05_materialization_receipt"):
        materialize_site05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
