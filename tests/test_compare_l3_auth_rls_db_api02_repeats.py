...
from scripts.compare_l3_auth_rls_db_api02_repeats import compare_api02_repeats, write_comparison
from scripts.materialize_l3_auth_rls_db_api02 import materialize_api02
...
def test_identical_api02_materializations_are_narrow_fix(tmp_path) -> None:
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    fixed = first[3] / "slots" / "api-02" / "candidate-0" / "fixed" / "app" / "api" / "users" / "[userId]" / "route.ts"
    fixed.write_text("tampered\n", encoding="ascii")
    with pytest.raises(ValueError, match="api02_materialization_source_content_invalid"):
        _compare(first, second)

    with pytest.raises(ValueError, match="api02_repeat_inputs_must_be_external"):
        compare_api02_repeats(
    ...
    with pytest.raises(ValueError, match="api02_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_api02_repeat_comparison"):
