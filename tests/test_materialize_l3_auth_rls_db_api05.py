...
from scripts.materialize_l3_auth_rls_db_api05 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api05_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-05"
    ...
    vulnerable = materialization_root / "slots" / "api-05" / "candidate-0" / "vulnerable" / "src" / "app" / "api" / "admin" / "audit" / "route.ts"
    fixed = materialization_root / "slots" / "api-05" / "candidate-0" / "fixed" / "src" / "app" / "api" / "admin" / "audit" / "route.ts"
    negative = materialization_root / "slots" / "api-05" / "candidate-1" / "negative" / "src" / "status.ts"
    reserve_vulnerable = materialization_root / "slots" / "api-05" / "candidate-1" / "vulnerable" / "src" / "app" / "api" / "admin" / "billing" / "route.ts"
    reserve_fixed = materialization_root / "slots" / "api-05" / "candidate-1" / "fixed" / "src" / "app" / "api" / "admin" / "billing" / "route.ts"
    assert b"export async function GET()" in vulnerable.read_bytes()
    assert b"const session = await getSession();" in vulnerable.read_bytes()
    assert b"session === null" in vulnerable.read_bytes()
    assert b'role === "admin"' not in vulnerable.read_bytes()
    assert b"adminAuditEvents" in vulnerable.read_bytes()
    assert b'role === "admin"' in fixed.read_bytes()
    assert b"hasAdminRole" in fixed.read_bytes()
    assert b"status: 403" in fixed.read_bytes()
    assert b"organizationBilling" in reserve_vulnerable.read_bytes()
    assert b'role === "admin"' not in reserve_vulnerable.read_bytes()
    assert b'role === "admin"' in reserve_fixed.read_bytes()
    assert b"status: 403" in reserve_fixed.read_bytes()
    assert b'export const status = "ok";' in negative.read_bytes()
    assert b"admin" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"adminAuditEvents" not in receipt_raw
    assert b"src/app/api/admin/audit/route.ts" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api05_materialization_candidates_invalid"):
    ...
    with pytest.raises(ValueError, match="api05_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    fixed.write_bytes(original_fixed)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api05_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api05_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api05_materialization_root_must_be_external"):
        materialize_api05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api05_materialization_receipt_output_must_be_external"):
        materialize_api05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api05_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api05_materialization_root"):
        materialize_api05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api05_materialization_receipt"):
        materialize_api05(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
