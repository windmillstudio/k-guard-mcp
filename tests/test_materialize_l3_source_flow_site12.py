...
from scripts.materialize_l3_source_flow_site12 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site12_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-12"
    ...
    vulnerable = materialization_root / "slots" / "site-12" / "candidate-0" / "vulnerable" / "routes" / "exports.py"
    fixed = materialization_root / "slots" / "site-12" / "candidate-0" / "fixed" / "routes" / "exports.py"
    negative = materialization_root / "slots" / "site-12" / "candidate-1" / "negative" / "routes" / "templates.py"
    pyproject = materialization_root / "slots" / "site-12" / "candidate-0" / "fixed" / "pyproject.toml"
    assert b"from flask import Response, request" in vulnerable.read_bytes()
    assert b'EXPORT_ROOT = Path("/srv/app/exports")' in vulnerable.read_bytes()
    assert b'request.args.get("file", "")' in vulnerable.read_bytes()
    assert b"(EXPORT_ROOT / file).read_text" in vulnerable.read_bytes()
    assert b'EXPORT_ROOT = Path("/srv/app/exports").resolve()' in fixed.read_bytes()
    assert b'candidate = (EXPORT_ROOT / file).resolve()' in fixed.read_bytes()
    assert b"if EXPORT_ROOT not in candidate.parents:" in fixed.read_bytes()
    assert b"return Response(status=404)" in fixed.read_bytes()
    assert b"request.args" not in negative.read_bytes()
    assert b'Response("Template list is available.", mimetype="text/plain")' in negative.read_bytes()
    assert b"Flask>=3.0,<4.0" in pyproject.read_bytes()
    assert b"request.args" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site12_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site12_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site12_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site12_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site12_materialization_root_must_be_external"):
        materialize_site12(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site12_materialization_receipt_output_must_be_external"):
        materialize_site12(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site12_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site12(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site12_materialization_root"):
        materialize_site12(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site12_materialization_receipt"):
        materialize_site12(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
