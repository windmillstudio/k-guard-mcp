...
from scripts.compare_l3_source_flow_site12_repeats import compare_site12_repeats, write_comparison
from scripts.materialize_l3_source_flow_site12 import materialize_site12
...
def test_identical_site12_materializations_are_narrow_fix(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    result = compare_site12_repeats(
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    (first[3] / "slots" / "site-12" / "candidate-0" / "fixed" / "routes" / "exports.py").write_text(
        "tampered\n", encoding="ascii"
    )
    with pytest.raises(ValueError, match="site12_materialization_source_content_invalid"):
        compare_site12_repeats(
            first_blueprint_path=first[0],
            first_staging_receipt_path=first[1],
            first_staging_root=first[2],
            first_materialization_root=first[3],
    ...
    with pytest.raises(ValueError, match="site12_repeat_inputs_must_be_external"):
        compare_site12_repeats(
    ...
    with pytest.raises(ValueError, match="site12_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_site12_repeat_comparison"):
