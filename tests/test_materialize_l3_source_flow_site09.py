...
from scripts.materialize_l3_source_flow_site09 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site09_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-09"
    ...
    vulnerable = materialization_root / "slots" / "site-09" / "candidate-0" / "vulnerable" / "app" / "checkout" / "page.tsx"
    fixed = materialization_root / "slots" / "site-09" / "candidate-0" / "fixed" / "app" / "checkout" / "page.tsx"
    negative = materialization_root / "slots" / "site-09" / "candidate-1" / "negative" / "app" / "integrations" / "page.tsx"
    assert b'"use client";' in vulnerable.read_bytes()
    assert b'const checkoutProviderKey = "k_guard_demo_client_key_primary";' in vulnerable.read_bytes()
    assert b'https://payments.example.invalid/v1/checkout' in vulnerable.read_bytes()
    assert b'Authorization: "Bearer " + checkoutProviderKey' in vulnerable.read_bytes()
    assert b'fetch("/api/checkout-session", { method: "POST" });' in fixed.read_bytes()
    assert b"Authorization" not in fixed.read_bytes()
    assert b"k_guard_demo_client_key_primary" not in fixed.read_bytes()
    assert b"Connect integration" in negative.read_bytes()
    assert b"fetch(" not in negative.read_bytes()
    assert b"k_guard_demo_client" not in negative.read_bytes()
    assert b"k_guard_demo_client" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site09_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site09_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site09_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site09_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site09_materialization_root_must_be_external"):
        materialize_site09(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site09_materialization_receipt_output_must_be_external"):
        materialize_site09(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site09_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site09(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site09_materialization_root"):
        materialize_site09(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site09_materialization_receipt"):
        materialize_site09(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
