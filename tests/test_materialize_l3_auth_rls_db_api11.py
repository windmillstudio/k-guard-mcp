...
from scripts.materialize_l3_auth_rls_db_api11 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api11_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-11"
    ...
    vulnerable = materialization_root / "slots" / "api-11" / "candidate-0" / "vulnerable" / "app" / "admin.py"
    fixed = materialization_root / "slots" / "api-11" / "candidate-0" / "fixed" / "app" / "admin.py"
    guards = materialization_root / "slots" / "api-11" / "candidate-0" / "fixed" / "app" / "guards.py"
    negative = materialization_root / "slots" / "api-11" / "candidate-1" / "negative" / "app" / "status.py"
    reserve_vulnerable = materialization_root / "slots" / "api-11" / "candidate-1" / "vulnerable" / "app" / "billing.py"
    reserve_fixed = materialization_root / "slots" / "api-11" / "candidate-1" / "fixed" / "app" / "billing.py"
    reserve_guards = materialization_root / "slots" / "api-11" / "candidate-1" / "fixed" / "app" / "guards.py"
    assert b'@bp.route("/admin/users/<user_id>", methods=["DELETE"])' in vulnerable.read_bytes()
    assert b"@require_admin" not in vulnerable.read_bytes()
    assert b"@require_admin" in fixed.read_bytes()
    assert b"runtime guard not materialized" in guards.read_bytes()
    assert b'@bp.route("/billing/refunds/<refund_id>", methods=["POST"])' in reserve_vulnerable.read_bytes()
    assert b"@require_finance_manager" not in reserve_vulnerable.read_bytes()
    assert b"@require_finance_manager" in reserve_fixed.read_bytes()
    assert b"runtime guard not materialized" in reserve_guards.read_bytes()
    assert b'APP_STATUS = "ok"' in negative.read_bytes()
    assert b"flask" not in negative.read_bytes()
    assert b"require_" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"require_admin" not in receipt_raw
    assert b"app/admin.py" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api11_materialization_candidates_invalid"):
    ...
    guards = materialization_root / "slots" / "api-11" / "candidate-0" / "fixed" / "app" / "guards.py"
    original_guards = guards.read_bytes()
    guards.write_text("def require_admin(function):\n    return function\n", encoding="ascii")
    with pytest.raises(ValueError, match="api11_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    guards.write_bytes(original_guards)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api11_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api11_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api11_materialization_root_must_be_external"):
        materialize_api11(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api11_materialization_receipt_output_must_be_external"):
        materialize_api11(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api11_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api11(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api11_materialization_root"):
        materialize_api11(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api11_materialization_receipt"):
        materialize_api11(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
