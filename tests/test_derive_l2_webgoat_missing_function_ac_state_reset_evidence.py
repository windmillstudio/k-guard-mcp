...
SCRIPT = ROOT / "scripts" / "derive_l2_webgoat_missing_function_ac_state_reset_evidence.py"
SPEC = importlib.util.spec_from_file_location("k_guard_l2_missing_function_ac_state_reset_test", SCRIPT)
...
def _source() -> dict:
    return {
        ...
        "source_receipt_sha256": adapter.SOURCE_RECEIPT_SHA256,
...
def _fixture_paths(
    ...
    monkeypatch.setattr(adapter, "POSITIVE_EXECUTION_RECEIPT_SHA256", adapter.sha256_bytes(positive_raw))
    ...
    _write(
        execution_path,
        {
            ...
            "claim_boundary": {"registry_evidence_integrated": False},
        },
    )
...
def test_rejects_cleanup_that_leaves_an_owned_volume(
    ...
    raw = _write(positive, payload)
    monkeypatch.setattr(adapter, "POSITIVE_EXECUTION_RECEIPT_SHA256", adapter.sha256_bytes(raw))
...
def test_rejects_execution_evidence_that_self_admits_registry_reset(
    ...
    payload["state_reset_evidence"]["registry_state_reset_admitted"] = True
