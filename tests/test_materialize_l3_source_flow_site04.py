...
from scripts.materialize_l3_source_flow_site04 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site04_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-04"
    ...
    vulnerable = materialization_root / "slots" / "site-04" / "candidate-0" / "vulnerable" / "app" / "api" / "report" / "route.ts"
    fixed = materialization_root / "slots" / "site-04" / "candidate-0" / "fixed" / "app" / "api" / "report" / "route.ts"
    negative = materialization_root / "slots" / "site-04" / "candidate-1" / "negative" / "app" / "api" / "download" / "route.ts"
    assert b"path.join(REPORT_ROOT, name)" in vulnerable.read_bytes()
    assert b"path.resolve(REPORT_ROOT, name)" in fixed.read_bytes()
    assert b"path.relative(REPORT_ROOT, candidate)" in fixed.read_bytes()
    assert b'path.join(EXPORT_ROOT, "terms.pdf")' in negative.read_bytes()
    assert b"request.url" not in negative.read_bytes()
    assert b"readFile" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site04_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site04_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site04_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site04_materialization_root_must_be_external"):
        materialize_site04(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site04_materialization_receipt_output_must_be_external"):
        materialize_site04(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site04_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site04(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site04_materialization_root"):
        materialize_site04(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site04_materialization_receipt"):
        materialize_site04(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
