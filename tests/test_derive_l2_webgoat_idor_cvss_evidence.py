...
SCRIPT = ROOT / "scripts" / "derive_l2_webgoat_idor_cvss_evidence.py"
SPEC = importlib.util.spec_from_file_location("k_guard_l2_webgoat_idor_cvss_evidence_test", SCRIPT)
...
def _selector() -> dict:
    return {
        ...
        "scenario_id": "webgoat:upstream-test-idor:deadbeefdeadbeef",
        "raw_returned": False,
    }
...
def _patch_dependencies(monkeypatch: pytest.MonkeyPatch, *, score: str = adapter.SCORE) -> None:
    ...
    cwe = {
        ...
        "classification": {"cwe": {"id": "CWE-639"}, "mechanism_truth": "present", "cvss_v4": None, "expected_disposition": None},
        "claim_boundary": {
    ...
    monkeypatch.setattr(adapter, "_load_execution_evidence", lambda _path: (execution, _sha("3"), _sha("4")))
    monkeypatch.setattr(adapter, "_calculator_provenance", lambda _root, _receipt: (calculator, _sha("5")))
    monkeypatch.setattr(adapter, "_calculate_score", lambda _root: score)


def test_derives_exact_raw_free_benchmark_cvss_profile(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_dependencies(monkeypatch)

    payload = adapter.derive_cvss_evidence(tmp_path / "cwe.json", tmp_path / "execution.json", tmp_path, tmp_path / "calculator.json")
...
def test_rejects_mismatched_source_selector(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_dependencies(monkeypatch)
    cwe, cwe_hash, cwe_adapter_hash = adapter._load_cwe_evidence(tmp_path / "cwe.json")
    execution, execution_hash, execution_adapter_hash = adapter._load_execution_evidence(tmp_path / "execution.json")
    execution["source"]["selector"]["scenario_id"] = "webgoat:upstream-test-idor:different"
    monkeypatch.setattr(adapter, "_load_cwe_evidence", lambda _path: (cwe, cwe_hash, cwe_adapter_hash))
    monkeypatch.setattr(adapter, "_load_execution_evidence", lambda _path: (execution, execution_hash, execution_adapter_hash))

    with pytest.raises(adapter.CvssEvidenceError, match="source_selector_mismatch"):
        adapter.derive_cvss_evidence(tmp_path / "cwe.json", tmp_path / "execution.json", tmp_path, tmp_path / "calculator.json")
...
def test_validator_rejects_forged_selector_profile_and_claim_boundary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_dependencies(monkeypatch)
    payload = adapter.derive_cvss_evidence(tmp_path / "cwe.json", tmp_path / "execution.json", tmp_path, tmp_path / "calculator.json")
