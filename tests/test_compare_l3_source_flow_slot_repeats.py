...
def test_identical_site02_materializations_are_narrow_fix(tmp_path) -> None:
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    (first[3] / "slots" / SLOT_ID / "candidate-0" / "fixed" / "routes" / "account.js").write_text(
        "tampered\n", encoding="ascii"
    )
    ...
    with pytest.raises(ValueError, match="source_flow_slot_repeat_output_must_be_external"):
