...
from scripts.materialize_l3_source_flow_site03 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site03_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-03"
    ...
    vulnerable = materialization_root / "slots" / "site-03" / "candidate-0" / "vulnerable" / "routes" / "archive.js"
    fixed = materialization_root / "slots" / "site-03" / "candidate-0" / "fixed" / "routes" / "archive.js"
    negative = materialization_root / "slots" / "site-03" / "candidate-1" / "negative" / "routes" / "export.js"
    assert b'const command = "archive --target " + target;' in vulnerable.read_bytes()
    assert b"return runner.exec(command);" in vulnerable.read_bytes()
    assert b'const allowed = new Set(["daily", "weekly"]);' in fixed.read_bytes()
    assert b'return runner.execFile("archive", ["--target", target]);' in fixed.read_bytes()
    assert b'return runner.execFile("export", ["--format", "json"]);' in negative.read_bytes()
    assert b"req.query" not in negative.read_bytes()
    assert b"runner.exec" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    tampered["candidates"][0]["transform_id"] = "different-transform"
    receipt_path.write_bytes(canonical_json_bytes(tampered))
    with pytest.raises(ValueError, match="site03_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site03_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site03_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site03_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site03_materialization_root_must_be_external"):
        materialize_site03(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site03_materialization_receipt_output_must_be_external"):
        materialize_site03(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site03_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site03(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site03_materialization_root"):
        materialize_site03(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site03_materialization_receipt"):
        materialize_site03(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
