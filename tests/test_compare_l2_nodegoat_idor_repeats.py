...
RUNNER_TEST_PATH = ROOT / "tests" / "test_replay_l2_nodegoat_idor.py"
RUNNER_TEST_SPEC = importlib.util.spec_from_file_location("nodegoat_replay_fixtures", RUNNER_TEST_PATH)
...
MODULE_PATH = ROOT / "scripts" / "compare_l2_nodegoat_idor_repeats.py"
SPEC = importlib.util.spec_from_file_location("k_guard_l2_nodegoat_idor_compare", MODULE_PATH)
...
def test_positive_comparator_ignores_nonces_but_binds_semantic_contract(tmp_path: Path) -> None:
    first = tmp_path / "positive-1.json"
    second = tmp_path / "positive-2.json"
    one = fixtures._positive_receipt()
    two = copy.deepcopy(one)
    one["runs"][0]["isolation"]["network"] = {"id_sha256": "1" * 64, "checks": {"internal": True}, "passed": True, "raw_returned": False}
    two["runs"][0]["run_nonce_sha256"] = "9" * 64
    two["runs"][0]["isolation"]["network"] = {"id_sha256": "9" * 64, "checks": {"internal": True}, "passed": True, "raw_returned": False}
    _write(first, one)
    _write(second, two)
...
def test_negative_comparator_requires_two_positive_receipt_anchors(tmp_path: Path) -> None:
    ...
    comparison = compare.compare_negative_receipts(negative_one, negative_two, positive_comparison)
    assert comparison["status"] == "FIX"
