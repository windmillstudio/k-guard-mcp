...
SLOT_ID = "data-14"
DATA09_IDENTITY_SHA256 = "85adae961398d163e9eb59378711f38f06ba53e68052f1fc6ac9b13d1e6bb5cd"
...
def test_materializes_only_data14_go_pgx_query_format_triplets(tmp_path) -> None:
    ...
    vulnerable = _source(materialization_root, 0, "vulnerable", "internal/checkout/invoices.go")
    fixed = _source(materialization_root, 0, "fixed", "internal/checkout/invoices.go")
    negative = _source(materialization_root, 1, "negative", "internal/catalog/products.go")
    go_mod = materialization_root / "slots" / SLOT_ID / "candidate-0" / "fixed" / "go.mod"
    assert b'mux.HandleFunc("/invoices/{number}", FindInvoiceByNumber(pool))' in vulnerable.read_bytes()
    assert b'number := r.PathValue("number")' in vulnerable.read_bytes()
    assert b'query := fmt.Sprintf("SELECT amount FROM invoices WHERE number = \'%s\'", number)' in vulnerable.read_bytes()
    assert b"pool.QueryRow(r.Context(), query)" in vulnerable.read_bytes()
    assert b'const query = "SELECT amount FROM invoices WHERE number = $1"' in fixed.read_bytes()
    assert b"pool.QueryRow(r.Context(), query, number)" in fixed.read_bytes()
    assert b"fmt.Sprintf" not in fixed.read_bytes()
    assert b'mux.HandleFunc("/products", FindProductBySKU())' in negative.read_bytes()
    assert b"func FindProductBySKU() http.HandlerFunc" in negative.read_bytes()
    assert b'[]byte("products-ready")' in negative.read_bytes()
    assert b"pgxpool" not in negative.read_bytes()
    assert b"github.com/jackc/pgx/v5 v5.7.2" in go_mod.read_bytes()
...
def test_data14_binding_is_explicit_and_data09_identity_is_unchanged() -> None:
    ...
    assert config["renderer"] == "go-pgx-query-format.v1"
    assert materializer_identity("data-09")["sha256"] == DATA09_IDENTITY_SHA256


def test_rejects_data14_receipt_source_role_root_and_staging_tamper(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)
...
def test_refuses_data14_repository_overlap_and_overwrite(tmp_path) -> None:
