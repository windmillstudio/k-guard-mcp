...
SCRIPT = ROOT / "scripts" / "compare_l1_benchmarkjava_denominator_repeats.py"
SPEC = importlib.util.spec_from_file_location("compare_l1_benchmarkjava_denominator_repeats_test", SCRIPT)
...
def _fixture_corpus(tmp_path: Path, language: str) -> tuple[Path, object]:
    ...
    _git(root, "config", "user.email", "p21a@example.invalid")
    _git(root, "config", "user.name", "P2.1A Fixture")
...
def _write_source_receipt(path: Path, java: Path, spec: object) -> None:
    materializer = comparison._load_materializer()
    blob_worktree = comparison._load_blob_worktree()
    expected = materializer._materialize_corpus(java, spec)
    receipt = {
...
def _manifest_pair(tmp_path: Path) -> tuple[Path, Path, Path, Path, object]:
    ...
    source_receipt = tmp_path / "java-source-receipt.json"
    _write_source_receipt(source_receipt, java, java_spec)
    return first, second, java, source_receipt, java_spec


def test_compares_equal_clean_java_denominator_manifests(tmp_path: Path) -> None:
    first, second, java, source_receipt, java_spec = _manifest_pair(tmp_path)
    ...
    assert result["field_id"] == "p2.1a-benchmarkjava-current-denominator"
    assert result["repeat_exact"] is True
    assert result["official_binding_exact"] is True
    assert result["benchmarkjava"]["case_count"] == 2
    assert result["target"]["head_git_oid"]
    assert len(result["tool_provenance"]["comparator_sha256"]) == 64
    assert result["authority"]["may_approve_benchmarkpython_denominator"] is False
    assert result["claim_boundary"]["detection_accuracy_proven"] is False
    assert str(java) not in comparison.canonical_json_bytes(result).decode("utf-8")


def test_holds_when_java_projection_differs_from_clean_source(tmp_path: Path) -> None:
    first, second, java, source_receipt, java_spec = _manifest_pair(tmp_path)
    materializer = comparison._load_materializer()
    altered = json_copy = json.loads(second.read_text(encoding="utf-8"))
    java_corpus = next(corpus for corpus in altered["corpora"] if corpus["language"] == "java")
    java_corpus["expected_results"]["sha256"] = "0" * 64
    second.write_bytes(materializer.canonical_json_bytes(json_copy))
...
def test_allows_a_distinct_lowercase_wbs_field_key_but_rejects_uppercase(tmp_path: Path) -> None:
    first, second, java, source_receipt, java_spec = _manifest_pair(tmp_path)
    ...
    result = comparison.compare_manifests(
        ...
        materializer_module=comparison._load_materializer(),
        java_spec=java_spec,
        field_id="p2.1b-benchmarkjava-independent-clean-worktree-replay",
    )

    assert result["field_id"] == "p2.1b-benchmarkjava-independent-clean-worktree-replay"
    with pytest.raises(ValueError, match="field_id_invalid"):
        comparison.compare_manifests(
            ...
            materializer_module=comparison._load_materializer(),
            java_spec=java_spec,
            field_id="P2.1B-invalid",
        )


def test_holds_when_source_receipt_is_valid_but_not_bound_to_java_source(tmp_path: Path) -> None:
    first, second, java, source_receipt, java_spec = _manifest_pair(tmp_path)
    blob_worktree = comparison._load_blob_worktree()
