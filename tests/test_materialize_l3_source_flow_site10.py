...
from scripts.materialize_l3_source_flow_site10 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site10_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-10"
    ...
    vulnerable = materialization_root / "slots" / "site-10" / "candidate-0" / "vulnerable" / "routes" / "checkout-cors.js"
    fixed = materialization_root / "slots" / "site-10" / "candidate-0" / "fixed" / "routes" / "checkout-cors.js"
    negative = materialization_root / "slots" / "site-10" / "candidate-1" / "negative" / "routes" / "integrations-cors.js"
    assert b"const origin = req.headers.origin;" in vulnerable.read_bytes()
    assert b'Access-Control-Allow-Origin", origin || "*"' in vulnerable.read_bytes()
    assert b'Access-Control-Allow-Credentials", "true"' in vulnerable.read_bytes()
    assert b'const allowedOrigins = new Set(["https://shop.example.invalid"]);' in fixed.read_bytes()
    assert b"allowedOrigins.has(origin)" in fixed.read_bytes()
    assert b'Access-Control-Allow-Credentials", "true"' in fixed.read_bytes()
    assert b'Vary", "Origin"' in fixed.read_bytes()
    assert b'origin || "*"' not in fixed.read_bytes()
    assert b"return res.status(204).end();" in negative.read_bytes()
    assert b"req.headers" not in negative.read_bytes()
    assert b"Access-Control" not in negative.read_bytes()
    assert b"Access-Control" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site10_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site10_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site10_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site10_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site10_materialization_root_must_be_external"):
        materialize_site10(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site10_materialization_receipt_output_must_be_external"):
        materialize_site10(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site10_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site10(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site10_materialization_root"):
        materialize_site10(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site10_materialization_receipt"):
        materialize_site10(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
