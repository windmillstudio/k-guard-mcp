...
from scripts.compare_l3_source_flow_site03_repeats import compare_site03_repeats, write_comparison
from scripts.materialize_l3_source_flow_site03 import materialize_site03
...
def test_identical_site03_materializations_are_narrow_fix(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    result = compare_site03_repeats(
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    (first[3] / "slots" / "site-03" / "candidate-0" / "fixed" / "routes" / "archive.js").write_text(
        "tampered\n", encoding="ascii"
    )
    with pytest.raises(ValueError, match="site03_materialization_source_content_invalid"):
        compare_site03_repeats(
            first_blueprint_path=first[0],
            first_staging_receipt_path=first[1],
            first_staging_root=first[2],
            first_materialization_root=first[3],
    ...
    with pytest.raises(ValueError, match="site03_repeat_inputs_must_be_external"):
        compare_site03_repeats(
    ...
    with pytest.raises(ValueError, match="site03_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_site03_repeat_comparison"):
