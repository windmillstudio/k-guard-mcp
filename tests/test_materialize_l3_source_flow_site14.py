...
from scripts.materialize_l3_source_flow_site14 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site14_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-14"
    ...
    build = materialization_root / "slots" / "site-14" / "candidate-0" / "fixed" / "build.gradle.kts"
    assert b"fun Route.receiptRoute()" in vulnerable.read_bytes()
    assert b'get("/receipt")' in vulnerable.read_bytes()
    assert b'queryParameters["message"]' in vulnerable.read_bytes()
    assert b'call.respondText("<section>$message</section>", ContentType.Text.Html)' in vulnerable.read_bytes()
    assert b"private fun escapeHtml" in fixed.read_bytes()
    assert b"val escapedMessage = escapeHtml(message)" in fixed.read_bytes()
    assert b'call.respondText("<section>$escapedMessage</section>", ContentType.Text.Html)' in fixed.read_bytes()
    assert b'queryParameters["message"]' not in negative.read_bytes()
    assert b'call.respondText("<section>invite-ready</section>", ContentType.Text.Html)' in negative.read_bytes()
    assert b"ktor-server-core-jvm" in build.read_bytes()
    assert b"kotlin(\"jvm\")" in build.read_bytes()
    assert b"<section>$message</section>" not in receipt_path.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site14_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site14_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_source_content_role_root_and_staging_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site14_materialization_source_content_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site14_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site14_materialization_root_must_be_external"):
        materialize_site14(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site14_materialization_receipt_output_must_be_external"):
        materialize_site14(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site14_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site14(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site14_materialization_root"):
        materialize_site14(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site14_materialization_receipt"):
        materialize_site14(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
