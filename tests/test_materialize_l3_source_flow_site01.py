...
from scripts.materialize_l3_source_flow_site01 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_site01_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "site-01"
    ...
    vulnerable = materialization_root / "slots" / "site-01" / "candidate-0" / "vulnerable" / "app" / "profile" / "page.tsx"
    fixed = materialization_root / "slots" / "site-01" / "candidate-0" / "fixed" / "app" / "profile" / "page.tsx"
    negative = materialization_root / "slots" / "site-01" / "candidate-1" / "negative" / "app" / "announcement" / "page.tsx"
    assert b"dangerouslySetInnerHTML" in vulnerable.read_bytes()
    assert b"dangerouslySetInnerHTML" not in fixed.read_bytes()
    assert b"Welcome to the announcement" in negative.read_bytes()
...
def test_rejects_receipt_and_source_tree_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site01_materialization_candidates_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site01_materialization_file_set_invalid"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="site01_materialization_root_binding_mismatch"):
        load_materialization_receipt(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site01_materialization_root_must_be_external"):
        materialize_site01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site01_materialization_receipt_output_must_be_external"):
        materialize_site01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="site01_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_site01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_site01_materialization_root"):
        materialize_site01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_site01_materialization_receipt"):
        materialize_site01(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
