...
SCRIPT = ROOT / "scripts" / "derive_l2_webgoat_sql_injection_advanced_cwe_evidence.py"
SPEC = importlib.util.spec_from_file_location(
    "k_guard_l2_webgoat_sql_injection_advanced_cwe_evidence_test", SCRIPT
)
...
TEST_SOURCE = b'''class SqlInjectionAdvancedIntegrationTest {
  void runTests() {
...
'''
IMPLEMENTATION_SOURCE = b'''class SqlInjectionLesson6a {
  void attack(String accountName) {
...
def test_derives_raw_free_source_bound_cwe_and_mechanism_evidence(
    ...
    assert payload["classification"]["cwe"]["id"] == "CWE-89"
    ...
    assert "user_system_data" not in rendered
...
def test_rejects_a_missing_test_anchor(
    ...
    broken = TEST_SOURCE.replace(b"SqlInjectionAdvanced/attack6a", b"SqlInjectionAdvanced/other")
...
def test_rejects_a_missing_implementation_anchor(
    ...
    broken = IMPLEMENTATION_SOURCE.replace(b"statement.executeQuery(query)", b"statement.executeQuery(safeQuery)")
...
def test_rejects_a_source_receipt_contract_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source_root, receipt_path = _fixture(tmp_path, monkeypatch)
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    ...
    with pytest.raises(adapter.CweEvidenceError, match="source_receipt_blob_truth_missing"):
        adapter.derive_cwe_evidence(source_root, receipt_path)
