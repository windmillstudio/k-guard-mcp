...
from scripts.compare_l3_source_flow_site13_repeats import compare_site13_repeats, write_comparison
from scripts.materialize_l3_source_flow_site13 import materialize_site13
...
def test_identical_site13_materializations_are_narrow_fix(tmp_path) -> None:
    first = _run(tmp_path, "first")
    second = _run(tmp_path, "second")
    result = compare_site13_repeats(
...
def test_comparison_rejects_tamper_repository_inputs_and_existing_output(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="site13_repeat_inputs_must_be_external"):
        compare_site13_repeats(
    ...
    with pytest.raises(ValueError, match="site13_repeat_output_must_be_external"):
        write_comparison(REPOSITORY_ROOT / "comparison.json", result)
    output = tmp_path / "comparison.json"
    output.write_text("existing\n", encoding="ascii")
    with pytest.raises(ValueError, match="refusing_to_overwrite_site13_repeat_comparison"):
