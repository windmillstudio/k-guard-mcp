...
from scripts.materialize_l3_auth_rls_db_api08 import (
    CLAIM_BOUNDARY,
...
def test_materializes_only_api08_primary_and_reserve_source_triplets(tmp_path) -> None:
    blueprint, stage_root, staging_receipt, materialization_root, receipt_path, receipt = _materialize(tmp_path)

    assert receipt["slot_id"] == "api-08"
    ...
    vulnerable = materialization_root / "slots" / "api-08" / "candidate-0" / "vulnerable" / "storage.rules"
    fixed = materialization_root / "slots" / "api-08" / "candidate-0" / "fixed" / "storage.rules"
    negative = materialization_root / "slots" / "api-08" / "candidate-1" / "negative" / "src" / "status.ts"
    reserve_vulnerable = materialization_root / "slots" / "api-08" / "candidate-1" / "vulnerable" / "storage.rules"
    reserve_fixed = materialization_root / "slots" / "api-08" / "candidate-1" / "fixed" / "storage.rules"
    assert b"match /uploads/{userId}/{allPaths=**} {" in vulnerable.read_bytes()
    assert b"allow read: if true;" in vulnerable.read_bytes()
    assert b"request.auth != null" not in vulnerable.read_bytes()
    assert b"allow read: if true;" not in fixed.read_bytes()
    assert b"request.auth != null && request.auth.uid == userId" in fixed.read_bytes()
    assert b"match /documents/{ownerId}/{allPaths=**} {" in reserve_vulnerable.read_bytes()
    assert b"allow read: if true;" in reserve_vulnerable.read_bytes()
    assert b"allow read: if true;" not in reserve_fixed.read_bytes()
    assert b"request.auth != null && request.auth.uid == ownerId" in reserve_fixed.read_bytes()
    assert b'export const status = "ok";' in negative.read_bytes()
    assert b"firebase" not in negative.read_bytes()
    receipt_raw = receipt_path.read_bytes()
    assert b"allow read: if true;" not in receipt_raw
    assert b"storage.rules" not in receipt_raw
...
def test_rejects_receipt_tree_and_source_role_tamper(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api08_materialization_candidates_invalid"):
    ...
    with pytest.raises(ValueError, match="api08_materialization_source_content_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)

    fixed.write_bytes(original_fixed)
    (materialization_root / "unexpected.txt").write_text("not allowed\n", encoding="ascii")
    with pytest.raises(ValueError, match="api08_materialization_file_set_invalid"):
        _load(blueprint, stage_root, staging_receipt, materialization_root, receipt_path)
...
def test_rejects_root_binding_and_staging_tree_tamper(tmp_path) -> None:
    ...
    other_root.mkdir()
    with pytest.raises(ValueError, match="api08_materialization_root_binding_mismatch"):
...
def test_refuses_repository_overlap_and_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="api08_materialization_root_must_be_external"):
        materialize_api08(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api08_materialization_receipt_output_must_be_external"):
        materialize_api08(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="api08_materialization_paths_must_not_overlap_inputs_or_receipt"):
        materialize_api08(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    existing_root.mkdir()
    with pytest.raises(ValueError, match="refusing_to_overwrite_api08_materialization_root"):
        materialize_api08(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
    ...
    with pytest.raises(ValueError, match="refusing_to_overwrite_api08_materialization_receipt"):
        materialize_api08(
            blueprint_path=blueprint,
            staging_receipt_path=staging_receipt,
            staging_root=stage_root,
