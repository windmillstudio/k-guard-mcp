...
SCRIPT = ROOT / "scripts" / "derive_l2_webgoat_missing_function_ac_cwe_evidence.py"
SPEC = importlib.util.spec_from_file_location(
    "k_guard_l2_webgoat_missing_function_ac_cwe_evidence_test", SCRIPT
)
...
TEST_SOURCE = b'''class AccessControlIntegrationTest {
  void testLesson() {
    startLesson("MissingFunctionAC", true);
    forbidden().statusCode(HttpStatus.SC_FORBIDDEN);
    String user = {"username":"%s","password":"%s","admin": "true"};
    client.post(webGoatUrlConfig.url("access-control/users"));
    client.get(webGoatUrlConfig.url("access-control/users-admin-fix"));
  }
}
'''
IMPLEMENTATION_SOURCE = b'''class MissingFunctionACUsers {
...
def _fixture(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        adapter._source_receipt_semantic_sha256(receipt),
    )
    blobs = {
...
def test_derives_raw_free_source_bound_cwe_and_mechanism_evidence(
    ...
    assert payload["classification"]["cwe"]["id"] == "CWE-266"
    ...
    assert "userRepository.save" not in rendered
    assert "stdout" not in rendered


def test_rejects_a_missing_test_anchor(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    broken = TEST_SOURCE.replace(b'admin": "true"', b'admin": "false"')
    source_root, receipt = _fixture(tmp_path, monkeypatch, test_source=broken)

    with pytest.raises(adapter.CweEvidenceError, match="test_source_anchor_missing"):
        adapter.derive_cwe_evidence(source_root, receipt)


def test_rejects_a_missing_implementation_anchor(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    broken = IMPLEMENTATION_SOURCE.replace(
        b"userRepository.save(newUser);", b"userRepository.save(safeUser);"
    )
...
def test_rejects_a_semantic_source_receipt_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source_root, receipt_path = _fixture(tmp_path, monkeypatch)
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    receipt["origin_repository_match"] = False
    raw = _write(receipt_path, receipt)
    monkeypatch.setattr(adapter, "SOURCE_RECEIPT_SHA256", adapter.sha256_bytes(raw))

    with pytest.raises(adapter.CweEvidenceError, match="source_receipt_semantic_identity_mismatch"):
        adapter.derive_cwe_evidence(source_root, receipt_path)
