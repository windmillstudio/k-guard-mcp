...
def test_build_contract_hash_excludes_runtime_nonce_but_binds_static_build_controls() -> None:
    baseline = idor._build_contract_sha256()

    assert baseline == idor._build_contract_sha256()
    assert baseline != idor._build_contract_sha256(contract_label="another-contract")
    assert baseline != idor._build_contract_sha256("FROM scratch\n")
