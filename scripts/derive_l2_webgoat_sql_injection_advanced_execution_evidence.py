...
SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_execution_evidence.v1"
APP_ID = "webgoat"
...
SOURCE_PATH = "src/it/java/org/owasp/webgoat/integration/SqlInjectionAdvancedIntegrationTest.java"
SOURCE_LINE = 13
SOURCE_CONTENT_SHA256 = "2ef8a359befd66f5a75cc4d30ce0be573c762eaf2f6d3bd052509afad2d7633a"
...
PROCESS_ARGV = (
    ...
    "-Dit.test=org.owasp.webgoat.integration.SqlInjectionAdvancedIntegrationTest",
    "verify",
)
...
def _load_replay_contract() -> tuple[Any, str]:
    path = Path(__file__).resolve(strict=True).with_name("replay_l2_webgoat_sql_injection_advanced.py")
    raw_before = path.read_bytes()
    name = "k_guard_l2_webgoat_sql_injection_advanced_execution_evidence_replay"
    ...
    except Exception as exc:  # pragma: no cover - protects a standalone CLI invocation.
...
def _validate_source_receipt(receipt: dict[str, Any], raw: bytes) -> None:
    ...
    } != {"sha256": SOURCE_CONTENT_SHA256, "byte_count": 2440}:
...
def _expected_process(*, expected_exit_code: int, expected_result_sha256: str) -> dict[str, Any]:
    return {
        ...
        "cwd": ".",
        "kind": "process",
        "network_policy": "isolated_loopback",
    ...
    *,
    ...
    for run in runs:
        ...
        if (
            ...
            or normalized.get("test_class") != "org.owasp.webgoat.integration.SqlInjectionAdvancedIntegrationTest"
        ...
        if require_control_triggered:
            if (
                run.get("expected_exit_code") != expected_exit_code
                or normalized.get("control_triggered") is not True
            ):
...
def _validate_execution_pair(
    ...
    if (
        ...
        or reference.get("source_receipt_sha256") != SOURCE_RECEIPT_RAW_SHA256
    ...
    if positive.get("image_cleanup", {}).get("passed") is not True or negative.get("image_cleanup", {}).get("passed") is not True:
...
def _state_reset_evidence(pair: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "negative_control_source_checkout_mutated": pair["negative_control_source_checkout_mutated"],
        "registry_state_reset_admitted": False,
        "raw_returned": False,
    }
...
def derive_execution_evidence(
    ...
    *,
    ...
    payload = {
        ...
        "execution_pair": pair,
        "oracle": _expected_process(expected_exit_code=0, expected_result_sha256=pair["positive_result_sha256"]),
...
def validate_execution_evidence(payload: dict[str, Any]) -> None:
    ...
    try:
        replay, replay_sha256 = _load_replay_contract()
    ...
    if (
        ...
        or any(pair.get(field) is not True for field in (
            "positive_per_run_cleanup_passed",
            "negative_per_run_cleanup_passed",
            "positive_image_cleanup_passed",
            "negative_image_cleanup_passed",
        ...
        or pair.get("negative_control_patch_id") != replay.NEGATIVE_CONTROL_PATCH_ID
        or pair.get("raw_returned") is not False
    ):
        raise ExecutionEvidenceError("execution_evidence_pair_invalid")
    _validate_process(payload.get("oracle"), expected_exit_code=0, expected_result_sha256=pair["positive_result_sha256"], label="oracle")
    _validate_process(payload.get("negative_control"), expected_exit_code=1, expected_result_sha256=pair["negative_result_sha256"], label="negative_control")
    expected_reset = _state_reset_evidence(pair)
    if payload.get("state_reset_evidence") != expected_reset:
...
def write_new_output(path: Path, payload: dict[str, Any]) -> Path:
    ...
    try:
        ...
        try:
            ...
            raise FileExistsError(f"refusing to overwrite execution evidence: {output}") from exc
    finally:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Derive raw-free WebGoat SQL Injection Advanced process evidence without admitting "
            "registry or release status."
        )
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        validate_execution_evidence(payload)
    except (ExecutionEvidenceError, FileExistsError) as exc:
        print(json.dumps({"status": "HOLD", "failure_code": str(exc)}, ensure_ascii=False, sort_keys=True))
        return 2
    print(json.dumps({"adapter_status": "EXECUTION_EVIDENCE_PASS", "release_gate_passed": False}, sort_keys=True))
    return 0
