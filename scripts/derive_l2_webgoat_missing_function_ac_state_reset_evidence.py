...
SCHEMA = "k_guard_l2_webgoat_missing_function_ac_state_reset_evidence.v1"
VALIDATION_SCHEMA = "k_guard_l2_webgoat_missing_function_ac_state_reset_validation.v1"
APP_ID = "webgoat"
...
def _load_execution_adapter() -> tuple[Any, str]:
    return _load_module(
        ...
        "k_guard_l2_missing_function_ac_state_reset_execution_adapter",
    )
...
def _expected_state_reset_command() -> dict[str, Any]:
    return {
        "argv": [
            "python",
            "scripts/derive_l2_webgoat_missing_function_ac_state_reset_evidence.py",
            "validate",
            "--evidence",
            "evidence/l2-webgoat-missing-function-ac-state-reset.json",
        ],
        "cwd": ".",
        "kind": "process",
...
def _validate_cleanup(receipt: dict[str, Any], *, label: str) -> tuple[list[str], str]:
    ...
    if not isinstance(image_id, str) or not _is_sha256(image_id.removeprefix("sha256:")):
        raise StateResetEvidenceError(f"{label}_image_identity_invalid")
    return nonces, sha256_bytes(image_id.encode("ascii"))
...
def derive_state_reset_evidence(
    ...
    *,
    ...
    execution_reset = execution.get("state_reset_evidence")
    if not isinstance(execution_reset, dict) or execution_reset.get("registry_state_reset_admitted") is not False:
    ...
    if execution_reset != execution_adapter._state_reset_evidence(pair):
        raise StateResetEvidenceError("execution_cleanup_projection_invalid")
    boundary = execution.get("claim_boundary")
    if not isinstance(boundary, dict) or boundary.get("registry_evidence_integrated") is not False:
        raise StateResetEvidenceError("execution_evidence_cannot_self_admit_registry")
    expected_source = {
        "app_id": APP_ID,
    ...
    if execution.get("source") != expected_source:
...
def validate_state_reset_evidence(payload: dict[str, Any]) -> None:
    ...
    if (
        ...
        or set(provenance) != {*provenance_fields, "raw_returned"}
        or any(not _is_sha256(provenance.get(field)) for field in provenance_fields)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Derive raw-free WebGoat Missing Function Access Control state-reset evidence "
            "without admitting registry or release status."
        )
    )
