...
MATERIALIZER_PATH = REPOSITORY_ROOT / "scripts" / "materialize_l3_source_flow_site03.py"
SCHEMA = "k_guard_l3_source_flow_site03_repeat_comparison.v1"
...
def _load_materializer() -> tuple[Any, str]:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l3_source_flow_site03_for_comparison", MATERIALIZER_PATH)
    if spec is None or spec.loader is None:
        raise ValueError("site03_materializer_unavailable")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    if MATERIALIZER_PATH.read_bytes() != raw_before:
        raise ValueError("site03_materializer_changed_while_loading")
...
def compare_site03_repeats(
    *,
    ...
    for path in paths:
        _require_external_path(path, "site03_repeat_inputs_must_be_external")
    ...
    return {
        ...
        "claim_boundary": {
            "site03_source_triplets_materialized": repeat_exact,
...
def write_comparison(output: Path, comparison: Mapping[str, Any]) -> None:
    _require_external_path(output, "site03_repeat_output_must_be_external")
    if output.exists() or output.is_symlink():
        raise ValueError("refusing_to_overwrite_site03_repeat_comparison")
    ...
    try:
        ...
        raise ValueError("refusing_to_overwrite_site03_repeat_comparison") from exc


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare two P2.4B.2.03 site-03 source-flow materializations.")
    ...
    try:
        comparison = compare_site03_repeats(
        ...
        print(f"compare_l3_source_flow_site03_repeats: {exc}", file=sys.stderr)
        return 2
