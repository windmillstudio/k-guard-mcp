...
SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_state_reset_evidence.v1"
VALIDATION_SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_state_reset_validation.v1"
APP_ID = "webgoat"
...
def _load_execution_adapter() -> tuple[Any, str]:
    return _load_module(
        ...
        "k_guard_l2_webgoat_sql_injection_advanced_state_reset_execution_adapter",
    )
...
def _expected_state_reset_command() -> dict[str, Any]:
    return {
        "argv": [
            "python",
            "scripts/derive_l2_webgoat_sql_injection_advanced_state_reset_evidence.py",
            "validate",
            "--evidence",
            "evidence/l2-webgoat-sql-injection-advanced-state-reset.json",
        ],
        "cwd": ".",
        "kind": "process",
...
def validate_state_reset_evidence(payload: dict[str, Any]) -> None:
    ...
    if (
        ...
        or not _is_sha256(inputs.get("positive_execution_receipt_sha256"))
        or not _is_sha256(inputs.get("negative_control_receipt_sha256"))
        or inputs.get("positive_execution_receipt_sha256")
        == inputs.get("negative_control_receipt_sha256")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Derive raw-free WebGoat SQL Injection Advanced state-reset evidence without admitting registry or release status."
    )
