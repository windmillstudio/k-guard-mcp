...
RUNNER_PATH = REPOSITORY_ROOT / "scripts" / "replay_l2_juice_shop_bola.py"
POSITIVE_SCHEMA = "k_guard_l2_juice_shop_bola_execution_repeat_comparison.v1"
NEGATIVE_SCHEMA = "k_guard_l2_juice_shop_bola_negative_control_repeat_comparison.v1"
...
def _load_runner() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l2_juice_shop_bola_repeat_runner", RUNNER_PATH)
...
def _projection_fields(value: Mapping[str, Any], fields: tuple[str, ...], label: str) -> dict[str, Any]:
...
def _base_projection(base: Mapping[str, Any]) -> dict[str, Any]:
    return _projection_fields(
        ...
        (
            ...
            "adapter_image_rootfs_layers_sha256",
            "adapter_digest",
...
def semantic_projection(receipt: Mapping[str, Any], *, negative: bool, positive_pair: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    projection = {
        ...
        "tool_provenance": _projection_fields(
            tool,
            (
                "runner_sha256",
    ...
    if negative:
        ...
        projection["negative_control"] = _projection_fields(
            control,
            (
                "patch_id",
                "source_path",
...
def compare_positive_receipts(first_path: Path, second_path: Path) -> dict[str, Any]:
    ...
    first_projection = semantic_projection(first, negative=False)
    second_projection = semantic_projection(second, negative=False)
    first_semantic_sha256 = sha256_bytes(first_projection)
    second_semantic_sha256 = sha256_bytes(second_projection)
    repeat_exact = (
        first_receipt_sha256 != second_receipt_sha256 and first_semantic_sha256 == second_semantic_sha256
    )
    return {
        "schema": POSITIVE_SCHEMA,
        "scenario": "juice-shop-basket-cross-user-read",
...
def _load_positive_comparison(
    ...
    if (
        comparison.get("scenario") != "juice-shop-basket-cross-user-read"
...
def compare_negative_receipts(
    ...
    first_projection = semantic_projection(first, negative=True, positive_pair=positive_pair)
    second_projection = semantic_projection(second, negative=True, positive_pair=positive_pair)
    first_semantic_sha256 = sha256_bytes(first_projection)
    second_semantic_sha256 = sha256_bytes(second_projection)
    repeat_exact = (
        first_receipt_sha256 != second_receipt_sha256 and first_semantic_sha256 == second_semantic_sha256
    ...
    return {
        ...
        "scenario": "juice-shop-basket-cross-user-read-negative-control",
        "first_receipt_sha256": first_receipt_sha256,
        "second_receipt_sha256": second_receipt_sha256,
        "first_semantic_fingerprint_sha256": first_semantic_sha256,
        "second_semantic_fingerprint_sha256": second_semantic_sha256,
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Compare two Juice Shop BOLA execution receipts without detector or release promotion."
    )
