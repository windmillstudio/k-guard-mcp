...
SCHEMA = "k_guard_l2_webgoat_missing_function_ac_cvss_evidence.v1"
CWE_EVIDENCE_SCHEMA = "k_guard_l2_webgoat_missing_function_ac_cwe_evidence.v1"
EXECUTION_EVIDENCE_SCHEMA = "k_guard_l2_webgoat_missing_function_ac_execution_evidence.v1"
...
EXPECTED_ROOT_CAUSE = (
    "upstream-test:org.owasp.webgoat.integration.AccessControlIntegrationTest#testLesson"
)
EXPECTED_SOURCE_PATH = (
    "src/it/java/org/owasp/webgoat/integration/AccessControlIntegrationTest.java"
)
...
def _load_cwe_adapter() -> tuple[Any, str]:
    return _load_module(
        "derive_l2_webgoat_missing_function_ac_cwe_evidence.py",
        "k_guard_l2_missing_function_ac_cvss_cwe",
    )


def _load_execution_adapter() -> tuple[Any, str]:
    return _load_module(
        "derive_l2_webgoat_missing_function_ac_execution_evidence.py",
        "k_guard_l2_missing_function_ac_cvss_execution",
    )


def _load_registry_contract() -> tuple[Any, str]:
    return _load_module(
        "materialize_l2_oracles.py", "k_guard_l2_missing_function_ac_cvss_registry"
    )
...
def _load_cwe_evidence(path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    if (
        ...
        or classification.get("cwe", {}).get("id") != "CWE-266"
        ...
        or boundary.get("source_bound_mechanism_truth_supported") is not True
...
def _selector_is_valid(selector: Any) -> bool:
    ...
    return (
        ...
        and not isinstance(selector.get("source_line"), bool)
...
def _calculator_provenance(
    calculator_root: Path, calculator_receipt: Path
...
const values = {AV:"N",AC:"L",AT:"N",PR:"L",UI:"N",VC:"L",VI:"H",VA:"N",SC:"N",SI:"N",SA:"N"};
...
def _metric_evidence() -> dict[str, str]:
    return {
        ...
        "AC": "deterministic_client_controlled_role_assignment_test",
        ...
        "VC": "privileged_user_listing_exposes_other_user_data",
        "VI": "client_controlled_admin_role_is_persisted",
...
def derive_cvss_evidence(
    ...
    execution, execution_sha256, execution_adapter_sha256 = _load_execution_evidence(
        execution_evidence_path
    )
    selector = _same_selector(cwe, execution)
    calculator, registry_contract_sha256 = _calculator_provenance(
        calculator_root, calculator_receipt_path
    )
...
def validate_cvss_evidence(payload: dict[str, Any]) -> None:
    ...
    if (
        ...
        or set(provenance) != {"adapter_sha256", "registry_contract_sha256", "raw_returned"}
        or provenance.get("adapter_sha256") != sha256_bytes(Path(__file__).read_bytes())
        or not _is_sha256(provenance.get("registry_contract_sha256"))
    ...
    if (
        ...
        or not _selector_is_valid(source.get("selector"))
        or any(
            not _is_sha256(source.get(key))
            for key in required_source - {"selector", "raw_returned"}
        )
    ):
    ...
    if (
        ...
        or profile.get("scope") != "pinned_webgoat_benchmark_scenario"
        or profile.get("vector") != VECTOR
        or profile.get("score") != SCORE
        or profile.get("severity") != SEVERITY
        or profile.get("expected_disposition") != EXPECTED_DISPOSITION
        or profile.get("metric_evidence") != _metric_evidence()
        or profile.get("raw_returned") is not False
    ):
        raise CvssEvidenceError("cvss_evidence_profile_invalid")
    calculator = profile.get("calculator")
    if (
        not isinstance(calculator, dict)
        or set(calculator)
        != {"id", "binding_sha256", "source_receipt_sha256", "raw_returned"}
        or calculator.get("id") != EXPECTED_CALCULATOR_ID
        or not _is_sha256(calculator.get("binding_sha256"))
        or not _is_sha256(calculator.get("source_receipt_sha256"))
        or calculator.get("raw_returned") is not False
    ):
        raise CvssEvidenceError("cvss_evidence_calculator_invalid")
    if (
        payload.get("claim_boundary") != _claim_boundary()
        or payload.get("admission_blockers") != ADMISSION_BLOCKERS
        or payload.get("adapter_status") != "CVSS_PROFILE_EVIDENCE_PASS"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Derive source-bound WebGoat Missing Function AC benchmark CVSS evidence"
    )
