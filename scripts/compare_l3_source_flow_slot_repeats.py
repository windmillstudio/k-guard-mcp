...
MATERIALIZER_PATH = REPOSITORY_ROOT / "scripts" / "materialize_l3_source_flow_slot.py"
...
def _load_materializer() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l3_source_flow_slot_for_comparison", MATERIALIZER_PATH)
    if spec is None or spec.loader is None:
        raise ValueError("source_flow_slot_materializer_unavailable")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    if MATERIALIZER_PATH.read_bytes() != raw_before:
        raise ValueError("source_flow_slot_materializer_changed_while_loading")
    return module


def _projection(receipt: Mapping[str, Any], identity: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "slot_definition_sha256": receipt["slot_definition_sha256"],
        "candidates": receipt["candidates"],
        "counts": receipt["counts"],
        "claim_boundary": receipt["claim_boundary"],
        "raw_source_returned": receipt["raw_source_returned"],
        "materializer_identity": dict(identity),
    }


def compare_source_flow_slot_repeats(
    *,
    slot_id: str,
    ...
    for path in paths:
        _require_external_path(path, "source_flow_slot_repeat_inputs_must_be_external")
    ...
    identity = materializer.materializer_identity(slot_id)
    first_projection = _projection(first, identity)
    second_projection = _projection(second, identity)
    ...
    input_receipt_schemas = {
        "first": {"schema": first["schema"], "version": first["version"]},
        "second": {"schema": second["schema"], "version": second["version"]},
    }
    legacy_receipt_migration_verified = any(
        item["schema"] == materializer.SCHEMA_V1 for item in input_receipt_schemas.values()
    )
    return {
        "schema": SCHEMA,
        "slot_id": slot_id,
        ...
        "materializer_identity": identity,
        "input_receipt_schemas": input_receipt_schemas,
        "legacy_receipt_migration_verified": legacy_receipt_migration_verified,
        "repeat_exact": repeat_exact,
        "status": "FIX" if repeat_exact else "HOLD",
        "authority": {
            "may_mark_field_fix": repeat_exact and not legacy_receipt_migration_verified,
            "may_revalidate_legacy_evidence": repeat_exact and legacy_receipt_migration_verified,
        ...
        "claim_boundary": {
            "source_triplets_materialized": repeat_exact,
...
def write_comparison(output: Path, comparison: Mapping[str, Any]) -> None:
    _require_external_path(output, "source_flow_slot_repeat_output_must_be_external")
    if output.exists() or output.is_symlink():
        raise ValueError("refusing_to_overwrite_source_flow_slot_repeat_comparison")
    ...
    try:
        ...
        raise ValueError("refusing_to_overwrite_source_flow_slot_repeat_comparison") from exc


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare two preregistered source-flow slot materializations.")
    ...
    try:
        comparison = compare_source_flow_slot_repeats(
        ...
        print(f"compare_l3_source_flow_slot_repeats: {exc}", file=sys.stderr)
        return 2
