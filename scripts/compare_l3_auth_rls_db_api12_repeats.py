...
MATERIALIZER_PATH = REPOSITORY_ROOT / "scripts" / "materialize_l3_auth_rls_db_api12.py"
SCHEMA = "k_guard_l3_auth_rls_db_api12_repeat_comparison.v1"
...
def _load_materializer() -> tuple[Any, str]:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l3_auth_rls_db_api12_for_comparison", MATERIALIZER_PATH)
    if spec is None or spec.loader is None:
        raise ValueError("api12_materializer_unavailable")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    if MATERIALIZER_PATH.read_bytes() != raw_before:
        raise ValueError("api12_materializer_changed_while_loading")
...
def compare_api12_repeats(
    *,
    ...
    for path in paths:
        _require_external_path(path, "api12_repeat_inputs_must_be_external")
    ...
    return {
        ...
        "claim_boundary": {
            "api12_source_triplets_materialized": repeat_exact,
...
def write_comparison(output: Path, comparison: Mapping[str, Any]) -> None:
    _require_external_path(output, "api12_repeat_output_must_be_external")
    if output.exists() or output.is_symlink():
        raise ValueError("refusing_to_overwrite_api12_repeat_comparison")
    ...
    try:
        ...
        raise ValueError("refusing_to_overwrite_api12_repeat_comparison") from exc


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare two P2.4B.3.12 api-12 Spring method-auth source materializations.")
    ...
    try:
        comparison = compare_api12_repeats(
        ...
        print(f"compare_l3_auth_rls_db_api12_repeats: {exc}", file=sys.stderr)
        return 2
