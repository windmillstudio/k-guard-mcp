...
RUNNER_PATH = REPOSITORY_ROOT / "scripts" / "replay_l2_pygoat_sensitive_data.py"
POSITIVE_SCHEMA = "k_guard_l2_pygoat_sensitive_data_execution_repeat_comparison.v1"
NEGATIVE_SCHEMA = "k_guard_l2_pygoat_sensitive_data_negative_control_repeat_comparison.v1"
SCENARIO = "pygoat-unauthenticated-all-users-sensitive-data-exposure"
...
def _load_runner() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l2_pygoat_sensitive_data_repeat_runner", RUNNER_PATH)
...
def _base_image_projection(value: Mapping[str, Any]) -> dict[str, Any]:
    return _fields(
        value,
        (
            "source_image_id",
...
def semantic_projection(
    ...
    if not all(isinstance(value, Mapping) for value in (source, tool, base_image, image)):
    ...
    projection = {
        ...
        "base_image": _base_image_projection(base_image),
...
def compare_negative_receipts(
    ...
    first_anchor = first.get("positive_execution_contract")
    second_anchor = second.get("positive_execution_contract")
    if not isinstance(first_anchor, Mapping) or not isinstance(second_anchor, Mapping):
        raise ValueError("negative_control_positive_anchor_invalid")
    first_positive_receipt_sha256 = first_anchor.get("receipt_sha256")
    second_positive_receipt_sha256 = second_anchor.get("receipt_sha256")
    if (
    ...
    first_fingerprint = sha256_value(
        semantic_projection(first, negative=True, positive_pair=positive_pair)
    )
    second_fingerprint = sha256_value(
        semantic_projection(second, negative=True, positive_pair=positive_pair)
    )
    repeat_exact = first_receipt_sha256 != second_receipt_sha256 and first_fingerprint == second_fingerprint
    return {
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Compare two raw-free PyGoat sensitive-data execution receipts."
    )
