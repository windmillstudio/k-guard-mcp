...
SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_execution_contract.v1"
RESULT_SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_normalized_result.v1"
NEGATIVE_CONTROL_SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_negative_control.v1"
NEGATIVE_CONTROL_RESULT_SCHEMA = "k_guard_l2_webgoat_sql_injection_advanced_negative_control_result.v1"
APP_ID = "webgoat"
...
EXPECTED_CASE_NAMES = ("runTests",)
SUCCESS_CASE_OUTCOMES = (("runTests", "pass"),)
NEGATIVE_CONTROL_CASE_OUTCOMES = (("runTests", "failure"),)
EXECUTION_CONTRACT_LABEL = "webgoat-sql-injection-advanced-v1"
NEGATIVE_CONTROL_LABEL = "webgoat-sql-injection-advanced-negative-control-v1"
...
def _apply_negative_control_patch(source_root: Path, verifier: Any) -> dict[str, Any]:
    ...
    vulnerable_marker = (
        b'      query = "SELECT * FROM user_data WHERE last_name = \'" + accountName + "\'";'
        + line_ending
    )
    vulnerable_replacement = line_ending.join(
        (
            b"      String escapedAccountName = accountName.replace(\"'\", \"''\");",
            b'      query = "SELECT * FROM user_data WHERE last_name = \'" + escapedAccountName + "\'";',
        )
    ) + line_ending
    if original.count(vulnerable_marker) != 1:
        raise RuntimeContractError("negative_control_patch_anchor_invalid")
    patched = original.replace(vulnerable_marker, vulnerable_replacement, 1)
    if patched == original:
    ...
    return {
        ...
        "patch_sha256": sha256_bytes(vulnerable_marker + b"\0" + vulnerable_replacement),
...
def _expected_tool_provenance() -> dict[str, Any]:
...
def execute_contract(source_root: Path, *, timeout: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="kguard-l2-webgoat-sql_injection_advanced-") as temporary:
...
def validate_receipt(receipt: dict[str, Any]) -> None:
    ...
    if receipt.get("tool_provenance") != _expected_tool_provenance():
...
def execute_negative_control(
    ...
    runs: list[dict[str, Any]] = []
    failure: str | None = None
    with tempfile.TemporaryDirectory(prefix="kguard-l2-webgoat-sql_injection_advanced-negative-control-") as temporary:
        work_root = Path(temporary)
        try:
...
def validate_negative_control_receipt(receipt: dict[str, Any]) -> None:
    ...
    if receipt.get("tool_provenance") != _expected_tool_provenance():
        raise RuntimeContractError("negative_control_tool_provenance_invalid")
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Replay one WebGoat SQL Injection Advanced execution contract without admitting scanner accuracy."
    )
