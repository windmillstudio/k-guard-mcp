...
SCHEMA = "k_guard_l2_webgoat_idor_cvss_evidence.v1"
CWE_EVIDENCE_SCHEMA = "k_guard_l2_webgoat_idor_cwe_evidence.v1"
EXECUTION_EVIDENCE_SCHEMA = "k_guard_l2_webgoat_idor_execution_evidence.v1"
...
def _load_cwe_adapter() -> tuple[Any, str]:
    return _load_module("derive_l2_webgoat_idor_cwe_evidence.py", "k_guard_l2_webgoat_idor_cvss_cwe")


def _load_execution_adapter() -> tuple[Any, str]:
    return _load_module("derive_l2_webgoat_idor_execution_evidence.py", "k_guard_l2_webgoat_idor_cvss_execution")


def _load_registry_contract() -> tuple[Any, str]:
    return _load_module("materialize_l2_oracles.py", "k_guard_l2_webgoat_idor_cvss_registry")
...
def _load_cwe_evidence(path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    if (
        ...
        or classification.get("cwe", {}).get("id") != "CWE-639"
...
def _selector_is_valid(selector: Any) -> bool:
    ...
    return (
        ...
        and isinstance(selector.get("root_cause"), str)
        and selector["root_cause"].startswith("upstream-test:")
        and isinstance(selector.get("source_path"), str)
        and selector["source_path"].endswith("IDORIntegrationTest.java")
        ...
        and selector["scenario_id"].startswith("webgoat:upstream-test-")
...
def _metric_evidence() -> dict[str, str]:
    return {
        "AV": "network_http_profile_endpoint",
        "AC": "deterministic_direct_object_reference_test",
        "AT": "no_preexisting_attack_condition_in_pinned_scenario",
        "PR": "authenticated_low_privilege_user_session",
        "UI": "no_victim_interaction_in_pinned_test",
        "VC": "cross_user_profile_disclosure",
        "VI": "cross_user_role_mutation",
...
def derive_cvss_evidence(
    ...
    payload = {
        ...
        "admission_blockers": ["registry_evidence_integration_missing", "negative_control_missing", "state_reset_missing"],
...
def validate_cvss_evidence(payload: dict[str, Any]) -> None:
    ...
    if payload.get("claim_boundary") != _claim_boundary() or payload.get("admission_blockers") != ["registry_evidence_integration_missing", "negative_control_missing", "state_reset_missing"] or payload.get("adapter_status") != "CVSS_PROFILE_EVIDENCE_PASS" or payload.get("release_gate_passed") is not False:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Derive source-bound WebGoat IDOR benchmark CVSS evidence")
