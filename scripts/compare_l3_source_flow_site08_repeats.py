...
MATERIALIZER_PATH = REPOSITORY_ROOT / "scripts" / "materialize_l3_source_flow_site08.py"
SCHEMA = "k_guard_l3_source_flow_site08_repeat_comparison.v1"
...
def _load_materializer() -> tuple[Any, str]:
    ...
    spec = importlib.util.spec_from_file_location("k_guard_l3_source_flow_site08_for_comparison", MATERIALIZER_PATH)
    if spec is None or spec.loader is None:
        raise ValueError("site08_materializer_unavailable")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    if MATERIALIZER_PATH.read_bytes() != raw_before:
        raise ValueError("site08_materializer_changed_while_loading")
...
def compare_site08_repeats(
    *,
    ...
    for path in paths:
        _require_external_path(path, "site08_repeat_inputs_must_be_external")
    ...
    return {
        ...
        "claim_boundary": {
            "site08_source_triplets_materialized": repeat_exact,
...
def write_comparison(output: Path, comparison: Mapping[str, Any]) -> None:
    _require_external_path(output, "site08_repeat_output_must_be_external")
    if output.exists() or output.is_symlink():
        raise ValueError("refusing_to_overwrite_site08_repeat_comparison")
    ...
    try:
        ...
        raise ValueError("refusing_to_overwrite_site08_repeat_comparison") from exc


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare two P2.4B.2.08 site-08 source-flow materializations.")
    ...
    try:
        comparison = compare_site08_repeats(
        ...
        print(f"compare_l3_source_flow_site08_repeats: {exc}", file=sys.stderr)
        return 2
